...
if __name__ == "__main__":
    ...
    for pm_file in module_config.pycometh_primary_relapse_file_hmm, module_config.pycometh_primary_relapse_file_cgi:
        ...
        samplecomp_hits += [
            ...
            for line in pm.read_file(b_minus_a=True, drop_insignificant=False, pval_threshold=0.05, min_diff=0.5)
        ]
    ...
    asm_hits = [
        {"chrom": line["chromosome"], "start": line["start"], "end": line["end"], "diff": line["diff"]}
        for line in pm_asm.read_file(b_minus_a=True, drop_insignificant=False, pval_threshold=0.05, min_diff=0.5)
    ]
    asm_hits = merge_duplicate_diffmet_hits(asm_hits)
    ...
    with pa.open_multipage_pdf("diffmet_segment_length"):
        ...
        lengths = np.log10([h["end"] - h["start"] for h in asm_hits])
        pa.figure()
        plt.hist(lengths, bins=200)
        plt.xlabel("Log 10 number of basepairs")
        plt.ylabel("Frequency")
