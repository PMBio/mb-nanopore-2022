...
do
	...
	then
	    ...
	    bcftools view ${INFILE} chr${CHR} | bcftools query -f "%POS[\t%GT\t%AD]\n" - | awk '$6=="0|1"' | sed 's/,/\t/g' | awk '$3+$4>0 {print $1"\t"$4/($3+$4)"\t"$8;}' | grep "0|1" | awk '{print $0"\t"(1-$2)"\t"$2;}' > partA
	    bcftools view ${INFILE} chr${CHR} | bcftools query -f "%POS[\t%GT\t%AD]\n" - | awk '$6=="1|0"' | sed 's/,/\t/g' | awk '$3+$4>0 {print $1"\t"$4/($3+$4)"\t"$8;}' | grep "1|0" | awk '{print $0"\t"$2"\t"(1-$2);}' > partB
